...
class Link : Serializable {
    ...
    constructor(json: JSONObject, normalizeHref: (String) -> String = { it }) {
        this.href = if(json.has("href")) json.getString("href") else throw JSONError.malformedJSON
        this.type = if(json.has("type")) json.getString("type") else null
        this.templated = if(json.has("templated")) json.getBoolean("templated") else false
        this.title = if(json.has("title")) json.getString("title") else null
        this.rels = if(json.has("rel")) parseArray<String>(json.getJSONArray("rel"), true).toMutableList() else mutableListOf()
        this.properties = if(json.has("properties")) Properties(json.getJSONObject("properties")) else Properties()
        ...
        this.bitrate = if(json.has("bitrate")) parsePositiveDouble(json.getDouble("bitrate")) as Double else null
